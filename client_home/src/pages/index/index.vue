<template>
  <view id="home" class="home--new">
    ...
    <Card
      class="list_diy"
      title="校园文章推荐"
      url="/pages/campus_articles/list"
      :list="list_article"
      v-if="$check_action('/campus_articles/list', 'get')"
    >
	  <list_campus_articles :list="list_campus_articles"></list_campus_articles>
...
<script>
							...
							import list_campus_articles from "@/components/diy/list_campus_articles.vue";
  ...
  methods: {
    ...
    sendChat() {
      ...
      this.sendValue = "";
		...
		*/
		get_campus_articles() {
			let url = "~/api/campus_articles/get_list?";
															url = "~/api/campus_articles/get_list?orderby=hits desc";
										
			this.$get(url, {
											}, (json) => {
					if (json.result && json.result.list) {
					console.log("校园文章" ,json.result.list);
					this.list_campus_articles = json.result.list;
											this.list_campus_articles.map((o)=>{
							o["praise_len"];
						});
						this.get_praise(this.list_campus_articles ,"campus_articles" ,"36227");
									}
			});
		},
