          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','repair_user') || ($check_field('add','repair_user') || $check_field('set','repair_user'))" label="报修用户" name="repair_user">
                  ...
                  v-if="user_group === '管理员' || (form['indoor_repair_report_id'] && $check_field('set','repair_user')) || (!form['indoor_repair_report_id'] && $check_field('add','repair_user'))"
                ></uni-data-select>
                <uni-data-select
                  v-model="form['repair_user']"
                  :localdata="list_user_repair_user"
              ...
              <uni-forms-item v-if="$check_field('get','full_name') || ($check_field('add','full_name') || $check_field('set','full_name'))" label="姓名" name="full_name">
                            <uni-easyinput type="text" v-model="form['full_name']" v-if="user_group === '管理员' || (form['indoor_repair_report_id'] && $check_field('set','full_name')) || (!form['indoor_repair_report_id'] && $check_field('add','full_name'))" :disabled="disabledObj['full_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','repair_date') || ($check_field('add','repair_date') || $check_field('set','repair_date'))" label="报修日期" name="repair_date">
                    <uni-datetime-picker v-if="user_group === '管理员' || (form['indoor_repair_report_id'] && $check_field('set','repair_date')) || (!form['indoor_repair_report_id'] && $check_field('add','repair_date'))" v-model="form['repair_date']" type="date" :disabled="disabledObj['repair_date_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','repair_date')">
                  {{ form['repair_date'] }}
                </text>
                  </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','select_region') || ($check_field('add','select_region') || $check_field('set','select_region'))" label="选择区域" name="select_region">
                    <uni-data-select
                  v-model="form.select_region"
                  :localdata="list_select_region"
                  :clear="!disabledObj['select_region_isDisabled']"
                  :disabled="disabledObj['select_region_isDisabled']"
                ...
                ></uni-data-select>
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','select_region')">
                  {{ form['select_region'] }}
                </text>
                  </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','specific_location') || ($check_field('add','specific_location') || $check_field('set','specific_location'))" label="具体位置" name="specific_location">
                            <uni-easyinput type="text" v-model="form['specific_location']" v-if="user_group === '管理员' || (form['indoor_repair_report_id'] && $check_field('set','specific_location')) || (!form['indoor_repair_report_id'] && $check_field('add','specific_location'))" :disabled="disabledObj['specific_location_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','specific_location')">
              ...
              <uni-forms-item v-if="$check_field('get','repair_content') || ($check_field('add','repair_content') || $check_field('set','repair_content'))" label="报修内容" name="repair_content">
                            <uni-easyinput type="text" v-model="form['repair_content']" v-if="user_group === '管理员' || (form['indoor_repair_report_id'] && $check_field('set','repair_content')) || (!form['indoor_repair_report_id'] && $check_field('add','repair_content'))" :disabled="disabledObj['repair_content_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','maintenance_records') || ($check_field('add','maintenance_records') || $check_field('set','maintenance_records'))" label="维修记录" name="maintenance_records">
                            <uni-easyinput type="text" v-model="form['maintenance_records']" v-if="user_group === '管理员' || (form['indoor_repair_report_id'] && $check_field('set','maintenance_records')) || (!form['indoor_repair_report_id'] && $check_field('add','maintenance_records'))" :disabled="disabledObj['maintenance_records_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','repair_personnel') || ($check_field('add','repair_personnel') || $check_field('set','repair_personnel'))" label="维修人员" name="repair_personnel">
                  ...
                  v-if="user_group === '管理员' || (form['indoor_repair_report_id'] && $check_field('set','repair_personnel')) || (!form['indoor_repair_report_id'] && $check_field('add','repair_personnel'))"
                ></uni-data-select>
                <uni-data-select
                  v-model="form['repair_personnel']"
                  :localdata="list_user_repair_personnel"
...
export default {
  ...
  methods: {
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/indoor_repair_report/upload?'),
     ...
     * 获取选择区域列表
     */
    async get_list_select_region() {
          ['教学楼','食堂','办公室','信息楼','其它区域'].map((o) => this.list_select_region.push({value:o,text:o}));
            },
