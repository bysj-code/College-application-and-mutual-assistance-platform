<template>
  <view class="page_student_articles diy_table diy_table_tml" id="student_articles_table">
    <!-- 筛选模块(开始) -->
    <view>
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/student_articles/view?" v-show="user_group == '管理员' || $check_action('/student_articles/table','add') || $check_action('/student_articles/view','add')">添加</navigator>
						...
						<view class="view">
							<view class="diy_title" v-if=" false">
								<span>文章封面</span>
									...
									<view class="col" v-if="0 && $check_field('get','release_students')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>发布学生</span>
							</view>
								...
								<span>
									{{ get_user_release_students(o['release_students']) }}
								</span>
								</view>
													</view>
					</view>
				...
				<view class="col">
					<view class="view">
						<view class="create_time_block diy_filed diy_time">
                          ...
                          <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/student_articles/table','del') || $check_action('/student_articles/view','del')">删除</button>
                        </view>
					</view>
				</view>
				<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/student_articles/view?' + field + '=' + o[field]"
						v-show="user_group == '管理员' || $check_action('/student_articles/table','set') || $check_action('/student_articles/view','set') || $check_action('/student_articles/view','get')" class="btn btn-primary">详情</navigator>
					                    									</view>
			</view>
		</view>
	</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 获取数据地址
				url_get_list: "~/api/student_articles/get_list?like=0",
				url_del: "~/api/student_articles/del?",
		...
		methods: {
          ...
          reset: function reset() {
            ...
            if(user_group != "管理员"){
                  let sqlwhere = "(";
                                                                                              if(user_group=="学生用户"){
                  sqlwhere+= "release_students = " + this.user.user_id + " or ";
              }
