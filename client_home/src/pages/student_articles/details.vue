<template>
	<view class="page_student_articles diy_detail diy_detail--new" id="student_articles_details">
		<view class="warp">
			<view class="container-fluid">
				<view class="row">
					...
					<view v-if="$check_field('get','release_students')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>发布学生</span>
							</view>
								<view class="diy_field diy_uid">
								{{ get_user_release_students(obj['release_students']) }}
							</view>
							</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			get_praise(obj) {
				...
				this.$get("~/api/praise/count?", {
					source_table: "student_articles",
					source_field: "student_articles_id",
					source_id: obj["student_articles_id"]
				}, (res) => {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "student_articles",
					source_field: "student_articles_id",
					source_id: this.obj["student_articles_id"],
					user_id
				};
        ...
        _this.obj.praise_len = parseInt(_this.obj.praise_len)
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/student_articles/set?student_articles_id=' + _this.obj["student_articles_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/student_articles/set?student_articles_id=' + obj["student_articles_id"], {
					hits
				}, (res) => {
					if(res.result){
