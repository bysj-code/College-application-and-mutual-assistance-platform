        ...
        <view>
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','job_id') || ($check_field('add','job_id') || $check_field('set','job_id'))" label="工号" name="job_id">
                            <uni-easyinput type="text" v-model="form['job_id']" v-if="user_group === '管理员' || (form['working_personnel_id'] && $check_field('set','job_id')) || (!form['working_personnel_id'] && $check_field('add','job_id'))" :disabled="disabledObj['job_id_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','job_id')">
                  {{ form['job_id'] }}
                </text>
                          </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','full_name') || ($check_field('add','full_name') || $check_field('set','full_name'))" label="姓名" name="full_name">
                            <uni-easyinput type="text" v-model="form['full_name']" v-if="user_group === '管理员' || (form['working_personnel_id'] && $check_field('set','full_name')) || (!form['working_personnel_id'] && $check_field('add','full_name'))" :disabled="disabledObj['full_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','gender') || ($check_field('add','gender') || $check_field('set','gender'))" label="性别" name="gender">
                  ...
                  v-if="user_group === '管理员' || (form['working_personnel_id'] && $check_field('set','gender')) || (!form['working_personnel_id'] && $check_field('add','gender'))"
                ></uni-data-select>
                <!-- 仅查看 -->
...
export default {
  ...
  data() {
    return {
      ...
      group_table: "working_personnel",
      is_password: true,
      field: "working_personnel_id",
      url_add: "~/api/working_personnel/add?",
      url_set: "~/api/working_personnel/set?",
      url_get_obj: "~/api/working_personnel/get_obj?",
      url_upload: "~/api/working_personnel/upload?",

      query: {
        "working_personnel_id": 0,
      },
      ...
      form: {
            "job_id":  '', // 工号
                    ...
                    "gender":  '', // 性别
                                "user_id": 0,
                "working_personnel_id": 0, // ID
                
              },
          disabledObj:{
                        "job_id_isDisabled": false,
  ...
  methods: {
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/working_personnel/upload?'),
    ...
    submit_sub(form_sub){
      if(form_sub["working_personnel_id"]){
        // 提交事件
        this.$post("~/api/" + this.group_table + "/set?working_personnel_id=" + form_sub["working_personnel_id"],form_sub,(res)=>{
    ...
    is_view(){
      ...
      if(!bl){
        bl = this.$check_action('/working_personnel/table','add');
        console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
      }
      if(!bl){
        bl = this.$check_action('/working_personnel/table','set');
        console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
      }
      if(!bl){
        bl = this.$check_action('/working_personnel/view','add');
        console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
      }
      if(!bl){
        bl = this.$check_action('/working_personnel/view','set');
        console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
      }
      if(!bl){
        bl = this.$check_action('/working_personnel/view','get');
