						...
						<view class="register_view">
							<uni-forms class="form_register" :rules="rules" :value="form" ref="form"
								...
								<uni-forms-item class="user_group" name="user_group" label="身份">
									...
									<!-- 学生用户 -->
								<form_student_users v-if="form.user_group === '学生用户'" @change="handleSubFormChange" :form="form_sub" ></form_student_users>
										<!-- 工作人员 -->
								<form_working_personnel v-if="form.user_group === '工作人员'" @change="handleSubFormChange" :form="form_sub" ></form_working_personnel>
...
<script>
	...
	import form_student_users from "@/components/diy/form_student_users";
			/* 工作人员 */
	import form_working_personnel from "@/components/diy/form_working_personnel";
			...
			async submit_check(param){
				...
				if(!ret && user_group){
						...
						var form_sub = Object.assign({} ,this.form_sub);
							...
							if(user_group === "工作人员"){
						var form_sub = Object.assign({} ,this.form_sub);
											if(!ret && !form_sub["job_id"]){
							ret = "工号不能为空!";
						}
						else if(!ret){
							res = await this.$get("~/api/working_personnel/count?", {"job_id": form_sub["job_id"]});
							if(res.result){
								ret = "工号已存在!";
							}
						}
