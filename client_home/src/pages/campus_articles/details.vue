<template>
	<view class="page_campus_articles diy_detail diy_detail--new" id="campus_articles_details">
		<view class="warp">
	...
	export default {
		...
		methods: {
			...
			get_praise(obj) {
				...
				this.$get("~/api/praise/count?", {
					source_table: "campus_articles",
					source_field: "campus_articles_id",
					source_id: obj["campus_articles_id"]
				}, (res) => {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "campus_articles",
					source_field: "campus_articles_id",
					source_id: this.obj["campus_articles_id"],
					user_id
				};
        ...
        _this.obj.praise_len = parseInt(_this.obj.praise_len)
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/campus_articles/set?campus_articles_id=' + _this.obj["campus_articles_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/campus_articles/set?campus_articles_id=' + obj["campus_articles_id"], {
					hits
				}, (res) => {
					if(res.result){
