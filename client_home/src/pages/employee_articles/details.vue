<template>
	<view class="page_employee_articles diy_detail diy_detail--new" id="employee_articles_details">
		<view class="warp">
			<view class="container-fluid">
				<view class="row">
					<view v-if="$check_field('get','article_title')" class="col-12 col-6">
						<view class="view">
					...
					<view v-if="$check_field('get','article_type')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>文章类型</span>
							</view>
					...
					<view v-if="$check_field('get','article_cover')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>文章封面</span>
							</view>
								<view class="diy_field diy_img">
								<img :src="$fullUrl(obj['article_cover'])" />
							</view>
							</view>
					</view>
					<view v-if="$check_field('get','release_employees')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>发布职工</span>
							</view>
								<view class="diy_field diy_uid">
								{{ get_user_release_employees(obj['release_employees']) }}
							</view>
							</view>
					</view>
					<view v-if="$check_field('get','article_content')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>文章内容</span>
							</view>
	...
	export default {
		...
		methods: {
			...
			get_praise(obj) {
				...
				this.$get("~/api/praise/count?", {
					source_table: "employee_articles",
					source_field: "employee_articles_id",
					source_id: obj["employee_articles_id"]
				}, (res) => {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "employee_articles",
					source_field: "employee_articles_id",
					source_id: this.obj["employee_articles_id"],
					user_id
				};
        ...
        _this.obj.praise_len = parseInt(_this.obj.praise_len)
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/employee_articles/set?employee_articles_id=' + _this.obj["employee_articles_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
								query.title = this.obj.article_title
																								query.img = this.obj.article_cover
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/employee_articles/set?employee_articles_id=' + obj["employee_articles_id"], {
					hits
				}, (res) => {
					if(res.result){
