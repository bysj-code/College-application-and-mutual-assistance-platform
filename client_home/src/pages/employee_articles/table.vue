<template>
  <view class="page_employee_articles diy_table diy_table_tml" id="employee_articles_table">
    <!-- 筛选模块(开始) -->
    <view>
          ...
          <view>
            <view class="">
              ...
              <uni-forms :modelValue="query">
                  <uni-forms-item label="文章标题" name="article_title">
                      <uni-easyinput type="text" v-model="query.article_title" placeholder="文章标题" />
                    </uni-forms-item>
                      <uni-forms-item label="文章类型" name="article_type">
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/employee_articles/view?" v-show="user_group == '管理员' || $check_action('/employee_articles/table','add') || $check_action('/employee_articles/view','add')">添加</navigator>
	...
	<view class="warp" >
		<view class="container">
			<view v-for="(o,i) in list" class="row box_style">
									<view class="col" v-if="1 && $check_field('get','article_title')">
						<view class="view">
							<view class="diy_title" v-if="true">
								...
								<span>
									...
									<view class="col" v-if="1 && $check_field('get','release_date')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>发布日期</span>
							</view>
								...
								<span>
									...
									<view class="col" v-if="1 && $check_field('get','article_type')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>文章类型</span>
							</view>
								...
								<span>
									...
									<view class="col" v-if="1 && $check_field('get','article_cover')">
						<view class="view">
							<view class="diy_title" v-if=" false">
								<span>文章封面</span>
							</view>
															<view class="diy_field diy_img">
									<img :src="$fullUrl(o['article_cover'])" width="100%" height="100" />
								</view>
													</view>
					</view>
									<view class="col" v-if="0 && $check_field('get','release_employees')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>发布职工</span>
							</view>
								...
								<span>
									{{ get_user_release_employees(o['release_employees']) }}
								</span>
								</view>
													</view>
					</view>
									<view class="col" v-if="0 && $check_field('get','article_content')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>文章内容</span>
							</view>
				...
				<view class="col">
					<view class="view">
						<view class="create_time_block diy_filed diy_time">
                          ...
                          <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/employee_articles/table','del') || $check_action('/employee_articles/view','del')">删除</button>
                        </view>
					</view>
				</view>
				<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/employee_articles/view?' + field + '=' + o[field]"
						v-show="user_group == '管理员' || $check_action('/employee_articles/table','set') || $check_action('/employee_articles/view','set') || $check_action('/employee_articles/view','get')" class="btn btn-primary">详情</navigator>
					                    									</view>
			</view>
		</view>
	</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 获取数据地址
				url_get_list: "~/api/employee_articles/get_list?like=0",
				url_del: "~/api/employee_articles/del?",
				...
				list: [],
                                    // 文章类型列表
                list_article_type: [{value:"",text:"全部"}],
                                                // 用户列表
		...
		methods: {
          ...
          reset: function reset() {
            ...
            if(user_group != "管理员"){
                  let sqlwhere = "(";
                                                                                              if(user_group=="工作人员"){
                  sqlwhere+= "release_employees = " + this.user.user_id + " or ";
              }
