          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','article_title') || ($check_field('add','article_title') || $check_field('set','article_title'))" label="文章标题" name="article_title">
                            <uni-easyinput type="text" v-model="form['article_title']" v-if="user_group === '管理员' || (form['employee_articles_id'] && $check_field('set','article_title')) || (!form['employee_articles_id'] && $check_field('add','article_title'))" :disabled="disabledObj['article_title_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','release_date') || ($check_field('add','release_date') || $check_field('set','release_date'))" label="发布日期" name="release_date">
                    <uni-datetime-picker v-if="user_group === '管理员' || (form['employee_articles_id'] && $check_field('set','release_date')) || (!form['employee_articles_id'] && $check_field('add','release_date'))" v-model="form['release_date']" type="date" :disabled="disabledObj['release_date_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','release_employees') || ($check_field('add','release_employees') || $check_field('set','release_employees'))" label="发布职工" name="release_employees">
                    <uni-data-select
                  id="form_release_employees"
                  v-model="form['release_employees']"
                  :localdata="list_user_release_employees"
                  :clear="!disabledObj['release_employees_isDisabled']"
                  :disabled="disabledObj['release_employees_isDisabled']"
                  v-if="user_group === '管理员' || (form['employee_articles_id'] && $check_field('set','release_employees')) || (!form['employee_articles_id'] && $check_field('add','release_employees'))"
                ></uni-data-select>
                <uni-data-select
                  v-model="form['release_employees']"
                  :localdata="list_user_release_employees"
                  :clear="false"
                  :disabled="true"
                  v-else-if="$check_field('get','release_employees')" id="release_employees"
                ></uni-data-select>
                  </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','article_content') || ($check_field('add','article_content') || $check_field('set','article_content'))" label="文章内容" name="article_content">
                    <uni-easyinput type="textarea" v-model="form['article_content']" v-if="user_group === '管理员' || (form['employee_articles_id'] && $check_field('set','article_content')) || (!form['employee_articles_id'] && $check_field('add','article_content'))" :disabled="disabledObj['article_content_isDisabled']" />
                <!-- 仅查看 -->
...
export default {
  ...
  methods: {
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/employee_articles/upload?'),
    ...
    async get_list_user_release_employees() {
      ...
      if(json.result && json.result.list){
        json.result.list.map((o) => this.list_user_release_employees.push({value:o.user_id,text:o.nickname + '-' + o.username}));
      }
      else if(json.error){
        console.error(json.error);
      }
    ...
    async get_group_user_release_employees() {
      ...
      if(json.result && json.result.obj){
        ...
        this.get_user_session_release_employees(this.form['release_employees'])
      }
      else if(json.error){
        console.error(json.error);
      }
