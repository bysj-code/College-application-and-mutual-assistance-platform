<template>
	<view class="page_employee_articles diy_list diy_list_page div_list_tml" id="employee_articles_list">
		<!-- 筛选模块(开始) -->
		<view>
			<view class="container">
				<view>
					<view>
						<view class="">
							<!-- 搜索栏 -->
													<uni-search-bar placeholder="搜索文章标题" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'article_title')">
			...
			<view class="container">
				<view v-for="(o,i) in list" class="row box_style">
					...
					<view class="uni-padding-wrap uni-common-mt">
						<navigator :url="'./details?employee_articles_id=' + o['employee_articles_id']" class="btn btn-primary">查看详情</navigator>
...
<script>
		...
		data() {
			return {
				url_get_list: "~/api/employee_articles/get_list?like=0",
									// 文章类型选项列表
				list_article_type: [{value:"全部",text:"全部"}],
											// 用户列表
	 ...
	 */
	get_praise(list){
		this.$get("~/api/praise/count_group?groupby=source_id&source_table=employee_articles", {}, (res) => {
			if(res.result){
				res.result.map((o)=>{
					for(var i = 0;i < list.length; i++){
						var oj = list[i];
						if(oj["employee_articles_id"] === o["source_id"]){
				...
				console.error(res.error);
						...
						// 改变分类标签
	change_article_type(val) {
		if (val === "全部" || val === "") {
			this.query.article_type = "";
		}else{
			this.query.article_type = val
		}
		this.search();
	},
