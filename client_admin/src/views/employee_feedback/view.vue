	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
				...
				<el-form-item label="反馈用户" prop="feedback_to_users">
							...
							<!--v-if="user_group === '管理员' || (form['employee_feedback_id'] && $check_field('set','feedback_to_users')) || (!form['employee_feedback_id'] && $check_field('add','feedback_to_users'))" :disabled="disabledObj['feedback_to_users_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','feedback_to_users')">{{form['feedback_to_users']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['employee_feedback_id'] && $check_field('set','feedback_to_users')) || (!form['employee_feedback_id'] && $check_field('add','feedback_to_users'))" id="feedback_to_users" v-model="form['feedback_to_users']" :disabled="disabledObj['feedback_to_users_isDisabled']">
				...
				<el-form-item label="姓名" prop="full_name">
							  ...
							  v-if="user_group === '管理员' || (form['employee_feedback_id'] && $check_field('set','full_name')) || (!form['employee_feedback_id'] && $check_field('add','full_name'))" :disabled="disabledObj['full_name_isDisabled']"></el-input>
				...
				<el-form-item label="反馈日期" prop="feedback_date">
								<el-date-picker :disabled="disabledObj['feedback_date_isDisabled']" v-if="user_group === '管理员' || (form['employee_feedback_id'] && $check_field('set','feedback_date')) || (!form['employee_feedback_id'] && $check_field('add','feedback_date'))" id="feedback_date"
				...
				<el-form-item label="反馈内容" prop="feedback_content">
						...
						v-if="user_group === '管理员' || (form['employee_feedback_id'] && $check_field('set','feedback_content')) || (!form['employee_feedback_id'] && $check_field('add','feedback_content'))" :disabled="disabledObj['feedback_content_isDisabled']"></el-input>
...
<script>
	...
	export default {
		...
		data() {
			return {
				field: "employee_feedback_id",
				url_add: "~/api/employee_feedback/add?",
				url_set: "~/api/employee_feedback/set?",
				url_get_obj: "~/api/employee_feedback/get_obj?",
				url_upload: "~/api/employee_feedback/upload?",

				query: {
					"employee_feedback_id": 0,
				},
		...
		methods: {
          ...
          this.form["feedback_date"] = null;
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/employee_feedback/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/employee_feedback/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/employee_feedback/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/employee_feedback/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/employee_feedback/view','get');
