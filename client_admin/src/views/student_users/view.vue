		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="学号" prop="student_id">
												<el-input id="student_id" v-model="form_sub['student_id']" placeholder="请输入学号"
							  v-if="user_group === '管理员' || (form_sub['student_users_id'] && $check_field('set','student_id')) || (!form_sub['student_users_id'] && $check_field('add','student_id'))"></el-input>
					<div v-else-if="$check_field('get','student_id')">{{form_sub['student_id']}}</div>
											</el-form-item>
			</el-col>
				...
				<el-form-item label="姓名" prop="full_name">
							  ...
							  v-if="user_group === '管理员' || (form_sub['student_users_id'] && $check_field('set','full_name')) || (!form_sub['student_users_id'] && $check_field('add','full_name'))"></el-input>
				...
				<el-form-item label="性别" prop="gender">
						...
						v-if="user_group === '管理员' || (form_sub['student_users_id'] && $check_field('set','gender')) || (!form_sub['student_users_id'] && $check_field('add','gender'))">
				...
				<el-form-item label="宿舍号" prop="dormitory_number">
												<el-input id="dormitory_number" v-model="form_sub['dormitory_number']" placeholder="请输入宿舍号"
							  v-if="user_group === '管理员' || (form_sub['student_users_id'] && $check_field('set','dormitory_number')) || (!form_sub['student_users_id'] && $check_field('add','dormitory_number'))"></el-input>
					<div v-else-if="$check_field('get','dormitory_number')">{{form_sub['dormitory_number']}}</div>
											</el-form-item>
			</el-col>
...
<script>
	...
	export default {
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["student_users_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?student_users_id=" + form_sub["student_users_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/student_users/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/student_users/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/student_users/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/student_users/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/student_users/view','get');
