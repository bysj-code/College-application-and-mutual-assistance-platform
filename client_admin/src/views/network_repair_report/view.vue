	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
				...
				<el-form-item label="报修用户" prop="repair_user">
							...
							<!--v-if="user_group === '管理员' || (form['network_repair_report_id'] && $check_field('set','repair_user')) || (!form['network_repair_report_id'] && $check_field('add','repair_user'))" :disabled="disabledObj['repair_user_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','repair_user')">{{form['repair_user']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['network_repair_report_id'] && $check_field('set','repair_user')) || (!form['network_repair_report_id'] && $check_field('add','repair_user'))" id="repair_user" v-model="form['repair_user']" :disabled="disabledObj['repair_user_isDisabled']">
				...
				<el-form-item label="姓名" prop="full_name">
							  ...
							  v-if="user_group === '管理员' || (form['network_repair_report_id'] && $check_field('set','full_name')) || (!form['network_repair_report_id'] && $check_field('add','full_name'))" :disabled="disabledObj['full_name_isDisabled']"></el-input>
				...
				<el-form-item label="报修日期" prop="repair_date">
								<el-date-picker :disabled="disabledObj['repair_date_isDisabled']" v-if="user_group === '管理员' || (form['network_repair_report_id'] && $check_field('set','repair_date')) || (!form['network_repair_report_id'] && $check_field('add','repair_date'))" id="repair_date"
						v-model="form['repair_date']" type="date" placeholder="选择日期">
					</el-date-picker>
					<div v-else-if="$check_field('get','repair_date')">{{form['repair_date']}}</div>
							</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','repair_location') || $check_field('add','repair_location') || $check_field('set','repair_location')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="报修位置" prop="repair_location">
												<el-input id="repair_location" v-model="form['repair_location']" placeholder="请输入报修位置"
							  v-if="user_group === '管理员' || (form['network_repair_report_id'] && $check_field('set','repair_location')) || (!form['network_repair_report_id'] && $check_field('add','repair_location'))" :disabled="disabledObj['repair_location_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','repair_location')">{{form['repair_location']}}</div>
											</el-form-item>
			</el-col>
				...
				<el-form-item label="报修内容" prop="repair_content">
							  ...
							  v-if="user_group === '管理员' || (form['network_repair_report_id'] && $check_field('set','repair_content')) || (!form['network_repair_report_id'] && $check_field('add','repair_content'))" :disabled="disabledObj['repair_content_isDisabled']"></el-input>
				...
				<el-form-item label="报修状态" prop="repair_status">
						...
						v-if="user_group === '管理员' || (form['network_repair_report_id'] && $check_field('set','repair_status')) || (!form['network_repair_report_id'] && $check_field('add','repair_status'))">
				...
				<el-form-item label="维修记录" prop="maintenance_records">
							  ...
							  v-if="user_group === '管理员' || (form['network_repair_report_id'] && $check_field('set','maintenance_records')) || (!form['network_repair_report_id'] && $check_field('add','maintenance_records'))" :disabled="disabledObj['maintenance_records_isDisabled']"></el-input>
				...
				<el-form-item label="维修人员" prop="repair_personnel">
													<el-select v-if="user_group === '管理员' || (form['network_repair_report_id'] && $check_field('set','repair_personnel')) || (!form['network_repair_report_id'] && $check_field('add','repair_personnel'))" id="repair_personnel" v-model="form['repair_personnel']" :disabled="disabledObj['repair_personnel_isDisabled']">
...
<script>
	...
	export default {
		...
		data() {
			return {
				field: "network_repair_report_id",
				url_add: "~/api/network_repair_report/add?",
				url_set: "~/api/network_repair_report/set?",
				url_get_obj: "~/api/network_repair_report/get_obj?",
				url_upload: "~/api/network_repair_report/upload?",
		...
		methods: {
          ...
          this.form["repair_date"] = null;
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/network_repair_report/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/network_repair_report/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/network_repair_report/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/network_repair_report/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/network_repair_report/view','get');
