		...
		<el-form label-position="right" :model="query" class="form p_4" label-width="120">
			<el-row>
					...
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/employee_articles/table','add') || $check_action('/employee_articles/view','add')" class="el-button el-button--default el-button--primary search_btn_add" to="./view?">添加
						</router-link>
            			<el-button v-if="user_group == '管理员' || $check_action('/employee_articles/table','del') || $check_action('/employee_articles/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			<el-table-column fixed type="selection" tooltip-effect="dark" width="55">
				...
				v-if="user_group == '管理员' || $check_field('get','article_cover')" min-width="200">
					...
					<el-table-column prop="release_employees" @sort-change="$sortChange" label="发布职工"
				v-if="user_group == '管理员' || $check_field('get','release_employees')" min-width="200">
						<template slot-scope="scope">
					{{ get_user_release_employees(scope.row['release_employees']) }}
				</template>
					</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/employee_articles/table','set') || $check_action('/employee_articles/view','set') || $check_action('/employee_articles/view','get') || $check_action('//table','add') || $check_action('//view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/employee_articles/table','set') || $check_action('/employee_articles/view','set') || $check_action('/employee_articles/view','get')"
