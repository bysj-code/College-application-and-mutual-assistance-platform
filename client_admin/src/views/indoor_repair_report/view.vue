	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
				...
				<el-form-item label="报修用户" prop="repair_user">
							...
							<!--v-if="user_group === '管理员' || (form['indoor_repair_report_id'] && $check_field('set','repair_user')) || (!form['indoor_repair_report_id'] && $check_field('add','repair_user'))" :disabled="disabledObj['repair_user_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','repair_user')">{{form['repair_user']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['indoor_repair_report_id'] && $check_field('set','repair_user')) || (!form['indoor_repair_report_id'] && $check_field('add','repair_user'))" id="repair_user" v-model="form['repair_user']" :disabled="disabledObj['repair_user_isDisabled']">
				...
				<el-form-item label="姓名" prop="full_name">
							  ...
							  v-if="user_group === '管理员' || (form['indoor_repair_report_id'] && $check_field('set','full_name')) || (!form['indoor_repair_report_id'] && $check_field('add','full_name'))" :disabled="disabledObj['full_name_isDisabled']"></el-input>
				...
				<el-form-item label="报修日期" prop="repair_date">
								<el-date-picker :disabled="disabledObj['repair_date_isDisabled']" v-if="user_group === '管理员' || (form['indoor_repair_report_id'] && $check_field('set','repair_date')) || (!form['indoor_repair_report_id'] && $check_field('add','repair_date'))" id="repair_date"
						v-model="form['repair_date']" type="date" placeholder="选择日期">
					</el-date-picker>
					<div v-else-if="$check_field('get','repair_date')">{{form['repair_date']}}</div>
							</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','select_region') || $check_field('add','select_region') || $check_field('set','select_region')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="选择区域" prop="select_region">
								<el-select id="select_region" v-model="form['select_region']"
						v-if="user_group === '管理员' || (form['indoor_repair_report_id'] && $check_field('set','select_region')) || (!form['indoor_repair_report_id'] && $check_field('add','select_region'))">
						<el-option v-for="o in list_select_region" :key="o" :label="o" :value="o">
						</el-option>
					</el-select>
					<div v-else-if="$check_field('get','select_region')">{{form['select_region']}}</div>
							</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','specific_location') || $check_field('add','specific_location') || $check_field('set','specific_location')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="具体位置" prop="specific_location">
												<el-input id="specific_location" v-model="form['specific_location']" placeholder="请输入具体位置"
							  v-if="user_group === '管理员' || (form['indoor_repair_report_id'] && $check_field('set','specific_location')) || (!form['indoor_repair_report_id'] && $check_field('add','specific_location'))" :disabled="disabledObj['specific_location_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','specific_location')">{{form['specific_location']}}</div>
											</el-form-item>
			</el-col>
				...
				<el-form-item label="报修内容" prop="repair_content">
							  ...
							  v-if="user_group === '管理员' || (form['indoor_repair_report_id'] && $check_field('set','repair_content')) || (!form['indoor_repair_report_id'] && $check_field('add','repair_content'))" :disabled="disabledObj['repair_content_isDisabled']"></el-input>
				...
				<el-form-item label="报修状态" prop="repair_status">
						...
						v-if="user_group === '管理员' || (form['indoor_repair_report_id'] && $check_field('set','repair_status')) || (!form['indoor_repair_report_id'] && $check_field('add','repair_status'))">
				...
				<el-form-item label="维修记录" prop="maintenance_records">
							  ...
							  v-if="user_group === '管理员' || (form['indoor_repair_report_id'] && $check_field('set','maintenance_records')) || (!form['indoor_repair_report_id'] && $check_field('add','maintenance_records'))" :disabled="disabledObj['maintenance_records_isDisabled']"></el-input>
				...
				<el-form-item label="维修人员" prop="repair_personnel">
													<el-select v-if="user_group === '管理员' || (form['indoor_repair_report_id'] && $check_field('set','repair_personnel')) || (!form['indoor_repair_report_id'] && $check_field('add','repair_personnel'))" id="repair_personnel" v-model="form['repair_personnel']" :disabled="disabledObj['repair_personnel_isDisabled']">
...
<script>
	...
	export default {
		...
		data() {
			return {
				field: "indoor_repair_report_id",
				url_add: "~/api/indoor_repair_report/add?",
				url_set: "~/api/indoor_repair_report/set?",
				url_get_obj: "~/api/indoor_repair_report/get_obj?",
				url_upload: "~/api/indoor_repair_report/upload?",

				query: {
					"indoor_repair_report_id": 0,
				},
				...
				disabledObj:{
								"repair_user_isDisabled": false,
										...
										"select_region_isDisabled": false,
										"specific_location_isDisabled": false,
				...
				group_user_repair_user: "",
						...
						// 选择区域选项列表
				list_select_region: ['教学楼','食堂','办公室','信息楼','其它区域'],
		...
		methods: {
          ...
          this.form["repair_date"] = null;
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/indoor_repair_report/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/indoor_repair_report/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/indoor_repair_report/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/indoor_repair_report/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/indoor_repair_report/view','get');
